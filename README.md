...
- **Destinations (implementations)**
  ...
  - **GCS Bucket** (`gcs_storage_pipeline.py`)
...
For storing the local directory into GCS bucket run the python script `gcs_storage_pipeline.py` locally outside of Docker, as long as the required GCP credentials are available locally or via environment variables.
...
**Google Cloud Storage bucket** is also used to simply uploads a local directory to GCS in `gcs_storage_pipeline.py`.
